...
app.use(require('cookie-parser')('6-1e8-4D_Z-1!t91@_aS.@l-x-IM2#4_1$-_"4_01/)+-nM_d;'));
...
app.get('/friends/:user' + nameregex + '/:app(\\d+)', function(req, res) {
  res.render('check.pug', {group: 'friends/' + req.params.user, app: req.params.app});
});

app.get('/friends/:user' + nameregex + '/check', auth.groupcheck);

app.get('/friends/:user' + nameregex, function(req, res) {
  res.render('wishlist.pug', {group: 'friends/' + req.params.user});
});

app.get('/people/:people/:app(\\d+)', function(req, res) {
  res.render('check.pug', {group: 'people/' + req.params.people, app: req.params.app});
});

app.get('/people/:people', function(req, res) {
  res.render('wishlist.pug', {group: 'people/' + req.params.people});
});

app.get('/people', auth.selectfriends);

app.get('/:group' + nameregex + '/:app(\\d+)', function(req, res) {
  res.render('check.pug', {group: req.params.group, app: req.params.app});
})

app.get('/:group' + nameregex + '/check', auth.groupcheck);

app.get('/:group' + nameregex, function(req, res) {
  res.render('wishlist.pug', {group: req.params.group});
});
